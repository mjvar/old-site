...
function toggleProjectPopup(proj){
	...
	if(proj == "aigh"){
		...
		popCont.innerHTML = `
				...
				<div class="research">
				<p>
					Specialized user interfaces can be desirable compared to generalized ones, as their design can be optimized for a given use case and user group. For example, it is much easier to generate gear outlines for laser cutting in a dedicated online gear-making tool than by manually drawing the gear in Adobe Illustrator. Unfortunately, creating specialized user interfaces is labor-intensive, and has to be done manually for each use-case. Our project explores automatically generating specialized drawing toolsets based on a dataset of desired laser cutouts. Our system is capable of creating parametrized shape tools that allow the user to quickly create multiple variations of the training shapes. Toolsets are generated by decomposing the vector training data into basic shapes, which are then used to parametrize the more complex figures. The best permutation of tools is selected based on a cost function that accounts for toolset complexity, workflow speed, and coverage.
				</p>
				</div>`;
	}
